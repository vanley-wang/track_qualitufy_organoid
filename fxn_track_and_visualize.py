...
def load_nifti(file_path):
    """加载 NIfTI 文件，返回数据和 affine（低内存占用）"""
    nii = nib.load(file_path)
    # 直接从 dataobj 读取，避免 nibabel 默认转为 float64
...
def instance_segmentation(binary_mask, min_voxel_count=cfg.MIN_VOXEL_COUNT):
    ...
    # 获取每个连通域的体积（使用 bincount 比 ndimage.sum 更快）
...
def save_seg_nrrd(data, output_path, colors=None, id_to_color_map=None):
    """
    保存为 3D Slicer 兼容的 .seg.nrrd 格式
    为每个 Segment 设置颜色和名称

    修复: 颜色由 Organoid_ID 哈希决定，而非排序索引，
         确保同一个 ID 在 Day3/Day5 始终显示相同颜色。
    """
    header = {
        'type': 'short',
        'dimension': 3,
    ...
    for seg_idx, label_value in enumerate(sorted(unique_labels)):
        ...
        # 用 label_value 决定颜色，而非 seg_idx，保证跨天颜色一致
        if id_to_color_map is not None and label_value in id_to_color_map:
            color = id_to_color_map[label_value]
        else:
            color = colors[label_value % n_colors]
...
def build_consistent_color_map(label3, label5, colors=None):
    """
    基于两天所有 label 的并集，建立一致的 label -> color 映射。
    这样即使某个 label 只在一天出现，两天中相同 label 的颜色也一致。
    """
    if colors is None:
        colors = cfg.SLICER_COLORS
    n_colors = len(colors)

    all_labels = set(np.unique(label3)) | set(np.unique(label5))
    all_labels = sorted([int(v) for v in all_labels if v > 0])

    id_to_color = {}
    for label_value in all_labels:
        id_to_color[label_value] = colors[label_value % n_colors]
    return id_to_color
...
def process_well(well_name, day3_seg_path, day5_seg_path, output_dir, max_distance=cfg.MAX_DISTANCE_PIXELS, generate_plots=True):
    ...
    for t_idx in unmatched:
        ...
        mapping[target_label] = next_new_id
        next_new_id += 1
    label5_matched = mapping[label5]

    # 预先构建 label -> prop 字典，加速日志查找
